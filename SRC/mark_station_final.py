...
def mark_stations_and_persons_on_video(video_path, stations, model, output_path=None, frame_skip=1, threshold=150):
    ...
    while cap.isOpened():
        ...
        for station in stations:
            ...
            for chair_position in station["Chair Positions"]:
        ...
        for ((x1, y1, x2, y2), label) in classified_persons:
            ...
            if label == "Sitting Person":
                ...
                if orientation == "Front":
                    eligible_stations = []
                    for station in stations:
                        station_y = station["Table Position"][1]
                        if station_y > y2:
                            dist = ((person_center[0] - station["Table Position"][0]) ** 2 +
                                    (person_center[1] - station_y) ** 2) ** 0.5
                            eligible_stations.append((dist, station))

                    if eligible_stations:
                        eligible_stations.sort(key=lambda tup: tup[0])
                        nearest_station = eligible_stations[0][1]
                        cv2.line(frame, person_center, nearest_station["Table Position"], (255, 0, 255), 2)
                        cv2.putText(frame, f"Assigned to {nearest_station['Station ID']}",
